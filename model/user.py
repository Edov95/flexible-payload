...
class User(object):
    ...
    def __init__(self):
        ...
        self._service_time = np.random.poisson(10)  # service time in seconds
...
class UserGenerationRate(object):
    ...
    def __init__(self):
        ...
        self._stamps = 400  # One sample every two minutes for 48h
    ...
    def step(self):
        ...
        Return the number of unser in the for the step. It is not related
        with the action taken.
        """
        # Numbers of arrivail in the timestep
        if self._time_stamp == self._stamps:
            self.calculate_lambda()
            self._time_stamp = 0
        # ret = np.random.poisson(self._arrival_rate[self._time_stamp])
        ret = int(self._arrival_rate[self._time_stamp])
        self._time_stamp += 1
        return ret
